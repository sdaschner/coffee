...
spec:
  containers:
  - name: coffee-shop
    image: registry.<region>.bluemix.net/istio-workshop/coffee-shop:1
    ports:
    - containerPort: 9080
...
Now, we want to configure coffee-shop to specify where the orders are from. We can do using Kubernetes configmap and MicroProfile Config.
...
The `@Inject` is CDI api while `@ConfigProperty` is an API in MicroProfile Config. This above will search `location` property from pre-defined config sources. If found, that value
will be injected to the variable location. Otherwise, the default value of HOME will be injected to location.

- Kubernetes configmap
  Kubernetes configmap is defined to store config properties. It can be mapped to environment variables.
...
location: JFokus
----
The above example defines the config map coffee-config, which contains config property `location` with the value of `JFokus`
env:
- name: location
  valueFrom:
    configMapKeyRef:
      name: coffee-config
      key: location
The above example maps the configmap to environment variable. As environment variables are default config sources for MicroProfile Config. All config defined here will be available
to MicroProfile Config lookup. Therefore, with this configuration, the injected value for the `location` will be `JFokus`.
...
With MicroProfile programming model together with Kubernetes, we can build 12-factor microservices easily.
