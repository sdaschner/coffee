...
spec:
  containers:
  - name: coffee-shop
    image: registry.eu-gb.bluemix.net/think-<your-name>-workshop/coffee-shop:1
    ports:
    - containerPort: 9080
...
This defines the config map `coffee-config`, which contains the property `location` with the value `Think`.
...
Thus, the injected value for the `location` will be the enum value `THINK`.
