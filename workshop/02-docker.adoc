...
----
cd coffee-shop/
docker build -t registry.eu-gb.bluemix.net/istio-workshop/coffee-shop:1 .
----

The image name, here `registry.eu-gb.bluemix.net/istio-workshop/coffee-shop:1`, implicitly contains the location of the image registry, image name, and version (or tag).
...
----
cd ../barista/
docker build -t registry.eu-gb.bluemix.net/istio-workshop/barista:1 .
----
...
docker run --rm -d \
  --name barista \
  --network dkrnet \
  registry.eu-gb.bluemix.net/istio-workshop/barista:1

docker run --rm -d \
  --name coffee-shop \
  --network dkrnet \
  -p 9080:9080 \
  registry.eu-gb.bluemix.net/istio-workshop/coffee-shop:1
----
...
----
docker push registry.eu-gb.bluemix.net/istio-workshop/coffee-shop:1
docker push registry.eu-gb.bluemix.net/istio-workshop/barista:1
----
