...
It offers capabilities for timeouts, retries, bulkheads, circuit breakers and fallbacks.
Fallbacks are one resiliency example which can only be achieved on an application-level, since we define how our application reacts to the exceptional situation.
